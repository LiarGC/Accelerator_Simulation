...
SBend::SBend(double length)
{
	this->length = length;
}

void SBend::SetTheta(double Theta)
{
	this->Theta = Theta;
...
void SBend::CalculateTransferMatrix()
{
	...
	else
	{
		double buffer[36] = {
			...
			0,					0,							1,				length,			0,		0,
			0,					0,							0,				1,				0,		0,
			-sin(Theta),		-(1 - cos(Theta)) * rho,	0,				0,				1,		-(beta * beta * length - sin(Theta) * rho),
			0,					0,							0,				0,				0,		1
		};
		TransferMatrix.InputMatrixBuffer(buffer, sizeof(buffer));
	}
...
void SBend::CalculateThroughB()
{
	SetTheta(- B * length / p_beam->p_IdealParticle->GetB_rho());
}

void SBend::CalculateThroughg()
{
	this->n = - rho / B * g;
}

Quad::Quad(double length)
{
	this->length = length;
}
